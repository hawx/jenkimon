    ...
    update: function() {
      ...
      $.ajax({
        ...
        for (var i = 0; i < data.jobs.length; i++) {
          ...
          if (opts.filters.length > 0 && !opts.filters.some(function (filter) { return opts.nameMatcher(job.name.toLowerCase(), filter); })) {
            continue;
          }
...
$(function() {
  ...
  builds.update();
  window.setInterval(builds.update, 5000);
});
